...
const MyInvoices = ({ navigateTo, db, appId, userId, pageContext }) => {
    ...
    const [activeTab, setActiveTab] = useState('readyToSend');
        ...
        try {
            ...
            if (db && appId) {
                try {
                    ...
                    if (invoiceDoc.exists()) completeInvoiceData = { id: invoice.id, ...invoiceDoc.data() };
            ...
            const previewDataObj = { customer: customerData, items: itemsArray, subtotal, taxes: safeTaxConfig, totals, invoiceId: completeInvoiceData.id, invoiceNumber: completeInvoiceData.invoiceNumber, invoiceDate: completeInvoiceData.invoiceDate, controllerSignature: completeInvoiceData.controllerSignature, controllerName: completeInvoiceData.controllerName, controllerSubsidiary: completeInvoiceData.controllerSubsidiary, signatureTimestamp: completeInvoiceData.signatureTimestamp, approvedBy: completeInvoiceData.approvedBy, currency: completeInvoiceData.currency, exchangeRate: completeInvoiceData.exchangeRate };
    ...
    const calculateDynamicTotals = (subtotalWithCharges, taxes, orderCharges = {}) => {
        ...
        totals.subtotal = subtotal; totals.shipping = orderCharges.shipping || 0; totals.handling = orderCharges.handling || 0; totals.discount = orderCharges.discount || 0; totals.subtotalWithCharges = subtotalWithCharges;
        let levyTotal = subtotalWithCharges;
        const subtotalTaxes = taxes.filter(t => t.on === 'subtotal' && t.enabled);
        subtotalTaxes.forEach(t => { const taxAmount = subtotalWithCharges * (t.rate / 100); totals[t.id] = taxAmount; totals[`${t.id}_rate`] = t.rate; levyTotal += taxAmount; });
        totals.levyTotal = levyTotal;
        const levyTaxes = taxes.filter(t => t.on === 'levyTotal' && t.enabled);
        let grandTotal = levyTotal;
        levyTaxes.forEach(t => { const taxAmount = levyTotal * (t.rate / 100); totals[t.id] = taxAmount; totals[`${t.id}_rate`] = t.rate; grandTotal += taxAmount; });
    ...
    const markAsSentToCustomer = async () => {
        ...
        try {
            ...
            await updateDoc(invoiceRef, { status: 'Awaiting Acceptance', sentAt: new Date() });
    ...
    const handleMarkAccepted = async (invoice) => {
        if (window.confirm(`Mark invoice ${invoice.approvedInvoiceId || invoice.id} as Accepted by Customer? This will recognize revenue.`)) {
            try {
                await updateDoc(doc(db, `artifacts/${appId}/public/data/invoices`, invoice.id), { status: 'Customer Accepted', customerActionAt: new Date() });
    ...
    const handleMarkRejected = async (invoice) => {
        ...
        if (!reason) return; // Exit if cancelled
        ...
        try {
            ...
            batch.update(invoiceRef, {
                status: 'Customer Rejected', // Matches the 'disputed' filter
            ...
            itemsToRestore.forEach(item => {
                // Only restore if it has an ID AND is NOT a sourced item
    ...
    const handleRevise = async (invoice) => {
        if (window.confirm(`Revise invoice ${invoice.approvedInvoiceId || invoice.id}? This will reset approval signatures, restore inventory, and move it to Draft.`)) {
            try {
                const batch = writeBatch(db);
                const invoiceRef = doc(db, `artifacts/${appId}/public/data/invoices`, invoice.id);
                batch.update(invoiceRef, { status: 'Draft', controllerSignature: null, approvedBy: null, signatureTimestamp: null, rejectionHistory: arrayUnion({ date: new Date().toISOString(), reason: invoice.rejectionReason || 'Manual Revision' }) });
                const itemsToRestore = invoice.items || invoice.lineItems || [];
                if (itemsToRestore.length > 0) {
                    ...
                    log('INVENTORY_ACTION', `Restored stock for revised Invoice ${invoice.id}`, { documentId: invoice.id, itemCount: itemsToRestore.length });
                }
                await batch.commit();
        ...
        <div className="min-h-screen bg-gray-100">
            <div className="max-w-7xl mx-auto p-4 md:p-8">
                ...
                <div className="flex space-x-1 rounded-xl bg-blue-900/20 p-1 mb-6">
                    <button onClick={() => setActiveTab('readyToSend')} className={`w-full rounded-lg py-2.5 text-sm font-medium leading-5 text-blue-700 ring-white ring-opacity-60 ring-offset-2 ring-offset-blue-400 focus:outline-none focus:ring-2 ${activeTab === 'readyToSend' ? 'bg-white shadow' : 'text-blue-100 hover:bg-white/[0.12] hover:text-white'}`}>Ready to Send</button>
                    <button onClick={() => setActiveTab('awaitingAcceptance')} className={`w-full rounded-lg py-2.5 text-sm font-medium leading-5 text-blue-700 ring-white ring-opacity-60 ring-offset-2 ring-offset-blue-400 focus:outline-none focus:ring-2 ${activeTab === 'awaitingAcceptance' ? 'bg-white shadow' : 'text-blue-100 hover:bg-white/[0.12] hover:text-white'}`}>Awaiting Acceptance</button>
                    <button onClick={() => setActiveTab('realizedRevenue')} className={`w-full rounded-lg py-2.5 text-sm font-medium leading-5 text-blue-700 ring-white ring-opacity-60 ring-offset-2 ring-offset-blue-400 focus:outline-none focus:ring-2 ${activeTab === 'realizedRevenue' ? 'bg-white shadow' : 'text-blue-100 hover:bg-white/[0.12] hover:text-white'}`}>Realized Revenue</button>
                    <button onClick={() => setActiveTab('disputed')} className={`w-full rounded-lg py-2.5 text-sm font-medium leading-5 text-blue-700 ring-white ring-opacity-60 ring-offset-2 ring-offset-blue-400 focus:outline-none focus:ring-2 ${activeTab === 'disputed' ? 'bg-white shadow' : 'text-blue-100 hover:bg-white/[0.12] hover:text-white'}`}>Disputed / Rejected</button>
                </div>
                            ...
                            <tbody>
                                {filteredInvoices.length === 0 ? (<tr><td colSpan="6" className="p-8 text-center text-gray-500">No invoices found in this category.</td></tr>) : (filteredInvoices.map(inv => (
                                    <tr key={inv.id} className="border-b hover:bg-gray-50">
                                        ...
                                        <td className="p-3 text-right">{(() => { const exchangeRate = inv.exchangeRate || 1; const displayTotal = inv.currency === 'USD' ? (inv.total / exchangeRate) : inv.total; return formatListAmount(displayTotal, inv.currency); })()}</td>
                                        <td className="p-3 text-center"><span className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(inv.status)}`}>{inv.status}</span></td>
                                        <td className="p-3 text-center space-x-2">
                                            ...
                                            {activeTab === 'realizedRevenue' && (<span className="text-xs text-green-600 font-medium"><Icon id="check" className="inline w-3 h-3 mr-1" /> Revenue Recognized</span>)}
                                        </td>
                                    </tr>
                                )))}
                            </tbody>
